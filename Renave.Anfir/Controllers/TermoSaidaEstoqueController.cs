...
{
    public class TermoSaidaEstoqueController : ApiController
    {
        ...
        public async Task<HttpResponseMessage> Get(int ID_Empresa, int idEstoque)
        {
            try
            {
                var url = basePath + "/api/montadora/estoques/" + idEstoque + "/termo-saida-estoque";
                ...
                using (var client = new HttpClient(handler))
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        var retorno = JsonConvert.DeserializeObject<TermoSaidaEstoque>(jsonString.Result);
